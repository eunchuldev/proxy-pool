...
async function timeout(promise, time) {
    ...
    const res = await Promise.race([
        new Promise((_, reject) => (handle = setTimeout(() => reject(new Error('timeout')), time))),
        promise,
    ]);
    if (handle)
        clearTimeout(handle);
...
class Worker {
    ...
    async _rotate() {
        ...
        await (stale === null || stale === void 0 ? void 0 : stale.deployment.destroy());
    }
    async rotate() {
        var _a, _b;
        try {
            ...
            while (this.deployments.length > 2)
                await ((_a = this.deployments.shift()) === null || _a === void 0 ? void 0 : _a.destroy());
        }
        if (this.destroyed)
            while (this.deployments.length)
                await ((_b = this.deployments.shift()) === null || _b === void 0 ? void 0 : _b.destroy());
    }
    async address() {
        var _a;
        return ((_a = this.deployments.peekFront()) === null || _a === void 0 ? void 0 : _a.address()) || null;
    }
    async destroy() {
        this.destroyed = true;
...
class MasterProxyServer {
    constructor(deploymentConstructor, option = new MasterProxyServerOption()) {
        ...
        this.workers = [...Array(this.option.maxWorkerCount).keys()].map(_ => new Worker(deploymentConstructor, this.option.maxWorkerAge));
        process.on('SIGTERM', () => {
    ...
    async nextProxyUrl(host) {
        ...
        return await this.workers[this.requestCountsByHost[host]++ % this.workers.length].address();
    }
    async listen() {
